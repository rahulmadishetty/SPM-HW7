...
class TestComputeTeamsVelocity(unittest.TestCase):
    ...
    def test_negative_sprint_points(self, mock_input):
        with patch('builtins.print') as mock_print:
            compute_team_velocity()
            mock_print.assert_called_with("Negative values for points are not allowed. Restart the program and input valid data.(Unhappy Path or negative scenarios)")

    @patch('builtins.input', side_effect=['0'])
    def test_zero_sprints(self, mock_input):
        with patch('builtins.print') as mock_print:
            compute_team_velocity()
            mock_print.assert_called_with("Please enter a positive integer for the number of sprints.(Unhappy Path or negative scenarios)")

    @patch('builtins.input', side_effect=['two', '10', '20'])
    def test_non_numeric_input_for_sprints(self, mock_input):
        with patch('builtins.print') as mock_print:
            compute_team_velocity()
            mock_print.assert_called_with("Please enter valid numerical inputs.(Unhappy Path or negative scenarios)")
